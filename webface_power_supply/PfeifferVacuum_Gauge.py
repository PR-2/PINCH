...
# Добавлена возможность выбора количества опрашиваемых каналов (строки 36, 37, 66, 69).
# by Bayander
...
class MaxiGauge(object):
    def __init__(self, serialPort, sensorNumber, baud=9600, debug=False):
    ...
    def pressures(self):
        return [self.pressure(i + 1) for i in
                range(self.sensorNumber)]  # Добавлена возможность выбора количества опрашиваемых каналов.

    def pressure(self, sensor):
        if sensor < 1 or sensor > self.sensorNumber: raise MaxiGaugeError(
            'Sensor can only be between 1 and 6. You choose ' + str(sensor))  # Добавлено "self.sensorNumber"
    ...
    def continuous_pressure_updates(self):
        ...
        while not self.stopping_continuous_update.isSet():
            ...
            cache.append([time.time()] + [sensor.pressure if sensor.status in [0, 1, 2] else float('nan') for sensor in
                                          self.cached_pressures])
    ...
    def log_to_file(self, logtime=None, logvalues=None):
        ...
        if not logtime:
            logtime = time.time()
        if not logvalues:
            logvalues = [sensor.pressure if sensor.status in [0, 1, 2] else float('nan') for sensor in
                         self.cached_pressures]
    ...
    def getACQorNAK(self):
        ...
        if len(returncode) < 3: self.debugMessage(
            'Only received a line termination from MaxiGauge. Was expecting ACQ or NAK.')
        ...
        if len(returncode) > 2 and returncode[-3] != C['ACQ']: self.debugMessage(
...
class PressureReading(object):
    def __init__(self, id, status, pressure):
        ...
        self.id = int(id)
        if int(status) not in PRESSURE_READING_STATUS.keys(): raise MaxiGaugeError(
            'The Pressure Status must be in the range %s' % PRESSURE_READING_STATUS.keys())
