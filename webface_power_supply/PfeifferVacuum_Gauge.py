...
class MaxiGauge (object):
    ...
    def pressures(self):
        return [self.pressure(i+1) for i in range(self.sensorNumber)] #Добавлена возможность выбора количества опрашиваемых каналов.

    def pressure(self, sensor):
        if sensor < 1 or sensor > self.sensorNumber: raise MaxiGaugeError('Sensor can only be between 1 and 6. You choose ' + str(sensor)) #Добавлено "self.sensorNumber"
